...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('identity_users', function (Blueprint $table) {
            ...
            $table->boolean('deleted')->default(false);
            $table->timestamps();
            $table->unique('username', 'identity_users_username_unique');
            $table->unique('email', 'identity_users_email_unique');
        ...
        Schema::create('user_passkeys', function (Blueprint $table) {
            $table->char('id', 36)->primary();
            $table->char('user_id', 36);
            $table->string('credential_id') /* unknown type: blob */;
            $table->string('credential_id_hash', 64)->nullable();
            $table->string('public_key') /* unknown type: blob */;
